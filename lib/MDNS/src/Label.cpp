...
uint8_t mdns::Label::Reader::next() {
  Log.trace(" mdns::Label::Reader::next: [1]");
  c = buffer->readUInt8();
  Log.trace(" mdns::Label::Reader::next: [2]");
  while ((c & LABEL_POINTER) == LABEL_POINTER) {
    Log.trace(" mdns::Label::Reader::next: [3]");

    if (buffer->available() > 0) {
      Log.trace(" mdns::Label::Reader::next: [4]");
      uint8_t c2 = buffer->readUInt8();
      Log.trace(" mdns::Label::Reader::next: [5]");
      uint16_t pointerOffset = ((c & ~LABEL_POINTER) << 8) | c2;
      Log.trace(" mdns::Label::Reader::next: [6]");
      buffer->mark();
      Log.trace(" mdns::Label::Reader::next: [7]");
      buffer->setOffset(pointerOffset);
      Log.trace(" mdns::Label::Reader::next: [8]");
      c = buffer->readUInt8();
      Log.trace(" mdns::Label::Reader::next: [9]");
    }
    else
    {
    ...
    Log.trace(" mdns::Label::Reader::next: [10]");
  }
  Log.trace(" mdns::Label::Reader::next: [11]");
  return c;
}
...
mdns::Label * mdns::Label::Matcher::match(std::map<String, Label *> labels, Buffer * buffer) {
  ...
  Log.trace(" mdns::Label::Matcher::match: [1]");
  Iterator * iterators[labels.size()];
  Log.trace(" mdns::Label::Matcher::match: [2]");
  std::map<String, Label *>::const_iterator i;

  uint8_t idx = 0;
  Log.trace(" mdns::Label::Matcher::match: [3]");
  for (i = labels.begin(); i != labels.end(); ++i) {
    iterators[idx++] = new Iterator(i->second);
  }
    Log.trace(" mdns::Label::Matcher::match: [4]");

  Reader * reader = new Reader(buffer);
  Log.trace(" mdns::Label::Matcher::match: [5]");
  while (reader->hasNext()) {
   Log.trace(" mdns::Label::Matcher::match: [6]");
    uint8_t size = reader->next();

    uint8_t idx = 0;
   Log.trace(" mdns::Label::Matcher::match: [7]");
    for (uint8_t i = 0; i < labels.size(); i++) {
      iterators[i]->match(size);
    }
   Log.trace(" mdns::Label::Matcher::match: [8]");
    while(idx < size && reader->hasNext()) {
     Log.trace(" mdns::Label::Matcher::match: [8.1]");
      uint8_t c = reader->next();
   Log.trace(" mdns::Label::Matcher::match: [8.2]");
      for (uint8_t i = 0; i < labels.size(); i++) {
  Log.trace(" mdns::Label::Matcher::match: [8.3]");
        iterators[i]->match(c);
      }
  Log.trace(" mdns::Label::Matcher::match: [8.4]");
      idx++;
    }
  Log.trace(" mdns::Label::Matcher::match: [9]");
  }

 Log.trace(" mdns::Label::Matcher::match: [10]");
  buffer->reset();
 Log.trace(" mdns::Label::Matcher::match: [11]");
  Label * label = NULL;
 Log.trace(" mdns::Label::Matcher::match: [12]");
  if (reader->endOfName()) {
    uint8_t idx = 0;
 Log.trace(" mdns::Label::Matcher::match: [13]");
    while (label == NULL && idx < labels.size()) {
  Log.trace(" mdns::Label::Matcher::match: [14]");
      if (iterators[idx]->matched()) {
        label = iterators[idx]->getStartLabel();
      }
   Log.trace(" mdns::Label::Matcher::match: [15]");
      idx++;
    }
  }
Log.trace(" mdns::Label::Matcher::match: [16]");
  for (uint8_t i = 0; i < labels.size(); i++) {
    delete iterators[i];
  }
Log.trace(" mdns::Label::Matcher::match: [17]");
  delete reader;
 Log.trace(" mdns::Label::Matcher::match: [18]");
  return label;
}
